...
const RegisterPage = () => {
  ...
  const initiateMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      if (data.exists) {
        ...
        setStep(2);
        showToast({ message: 'OTP sent to your email.' });
      }
    },
  ...
  const completeMutation = useMutation({
    ...
    onSuccess: () => {
      showToast({ message: 'Registration complete! You can now log in.', type: 'success' });
      navigate('/login');
    },
    onError: (error: any) => {
  ...
  const submitOtp = () => {
    ...
    if (otpCode.length === 6) verifyMutation.mutate({ email: registeredEmail, otp: otpCode });
    else showToast({ message: 'Please enter all 6 digits', type: 'error' });
  };

  const onSubmitStep1 = (data: any) => initiateMutation.mutate({ email: data.email });
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="w-full bg-surface-container-lowest p-8 md:p-10 rounded-xl shadow-sm border border-outline-variant/15 transition-all">
        ...
        {step === 1 && (
          <>
            <h2 className="text-xl font-bold text-on-surface mb-6">Register an account</h2>
            {!userExists ? (
              ...
              <div className="space-y-4">
                ...
                <button onClick={() => navigate('/login')} className="w-full h-12 bg-primary text-white font-bold rounded-xl">
                  Go to Login
                </button>
              </div>
            )}
        ...
        {step === 2 && (
          <div className="space-y-6">
            <div>
              <h2 className="text-xl font-bold text-on-surface">Verify your email</h2>
              <p className="text-sm text-on-surface-variant">We sent an OTP to {registeredEmail}</p>
            </div>
            <div className="flex justify-center gap-2">
              {otp.map((digit, i) => (
                <input key={i} id={`otp-${i}`} type="text" value={digit}
                  onChange={(e) => handleOtpChange(i, e.target.value)}
                  onKeyDown={(e) => handleOtpKeyDown(i, e)}
                  maxLength={1}
                  className="w-12 h-14 text-center text-xl font-bold bg-surface-container-high border rounded-lg focus:border-primary focus:ring-1 outline-none"
                />
              ))}
            </div>
            <button onClick={submitOtp} disabled={verifyMutation.isPending} 
              className="w-full h-12 gradient-btn text-white font-bold rounded-xl shadow-md hover:shadow-xl transition-all disabled:opacity-70">
        ...
        {step === 3 && (
          <>
            <h2 className="text-xl font-bold text-on-surface mb-6">Complete Profile</h2>
            <form onSubmit={handleSubmit(onSubmitStep3)} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2 bg-surface-container p-4 rounded-xl">
                {constraints.map((c, i) => (
